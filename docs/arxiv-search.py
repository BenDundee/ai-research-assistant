# https://arxivxplorer.com/?q=multi+agent+chatbots

import requests

#url = "https://arxivxplorer.com/?q=multi+agent+chatbots"
...
set_kaggle_credentials("bendundee", "eb225ca553e1e6defcf704eec38d6118")
path = kagglehub.dataset_download("tomtum/openai-arxiv-embeddings")
print("wait")

with self.embeddings_txt.open("r", newline="") as f:
    reader = csv.reader(f)
    iterator = tqdm(reader, total=expected_rows, desc="Embedding lines") \
        if self.show_progress else reader
    ...
    for idx, row in enumerate(iterator):
        ...
        if self._dim is None:
            ...
            if self._dim != 3_072:
                logger.warning("Unexpected embedding size: %d", self._dim)

        meta_row = df.iloc[idx]
        paper_id = str(meta_row["id"])

        batch_ids.append(paper_id)
        batch_vectors.append(vector)
        batch_meta.append({"journal_name": meta_row["journal_name"]})

        if len(batch_ids) >= self.batch_size:
            self._add_batch(batch_ids, batch_vectors, batch_meta)
